...
const EmployeeAttendanceAndSalaryReportsViewOwner = () => {
  ...
  const calculateTotalSalary = (data) => {
    return data.reduce((acc, item) => acc + (item.salary || 0), 0);
  };
              ...
              <tbody>
                {initValue.map((item) => (
                  <tr key={item.employee_id}>
                    ...
                    <td>{item.attend}</td>
                    <td>{item.absent}</td>
                    <td>{(item.salary || 0).toLocaleString("id-ID")}</td>
                    <td>
                    ...
                    <td>
                      {(
                        (item.salary || 0) + (parseInt(item.daily_bonus) || 0)
...
export default EmployeeAttendanceAndSalaryReportsViewOwner;
