...
const PurchaseIngredientView = () => {
  const initValue = useSelector(
    (state) => state.purchaseIngredientStore.purchaseIngredientData
  );
  ...
  let totalPrice = 0; // Ubah const menjadi let karena nilai totalPrice akan berubah

  initValue.forEach((e) => {
    totalPrice += e.total_price; // Tambahkan nilai total_price ke totalPrice
  });
        ...
        >
          <div className="card shadow-lg p-4 mb-5 rounded">
            ...
            <table className="table">
              ...
              <tbody>
                {(searchTerm ? filteredPurchaseIngredient : initValue).map(
                  (p) => {
                    noUrut += 1;
                    return (
